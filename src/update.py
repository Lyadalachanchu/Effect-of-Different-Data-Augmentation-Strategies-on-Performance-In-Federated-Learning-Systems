...
from impute import impute_naive, impute_cvae_naive, impute_cvae_minority
...
class LocalUpdate(object):
    ...
    def train_val_test(self, dataset, idxs):
        ...
        checkpoint = torch.load(f'C:\\Users\\LohithSai\\Desktop\\FederatedImputation\\vae_data\\models\\0_cvae_{self.args.dirichlet}_cvae.pth')
        trained_cvae.load_state_dict(checkpoint)
        print(f"minority: {self.args.minority}")
        # generated_train_dataset = impute_naive(k=self.args.num_generate, trained_vae=trained_vae, initial_dataset=train_dataset)
        if self.args.minority == True:
            print("MINORITY IS TRUE")
            generated_train_dataset = impute_cvae_minority(k=self.args.num_generate, trained_cvae=trained_cvae, initial_dataset=train_dataset)

        else:
            print("MINORITY IS FALSE")
        ...
        train_dataset = [(torch.tensor(image), torch.tensor(label)) for image, label in
                                           generated_train_dataset]
        trainloader = DataLoader(train_dataset,
                                 batch_size=self.args.local_bs, shuffle=True)
        validloader = DataLoader(DatasetSplit(dataset, idxs_val),
                                 batch_size=int(self.args.local_bs), shuffle=True)
    ...
    learning_rate=0.001
            ...
            for iter in range(self.args.local_ep):
                ...
                for batch_idx, (images, labels) in enumerate(self.trainloader):
                    ...
                    while True:
                        if not torch.isnan(log_probs).any().item():
                            break
                    ...
                    if self.args.verbose and (batch_idx % 10 == 0):
                        ...
                        print(f"log_probs: {log_probs[:1]}\t labels: {labels[:1]}")
